...
TransmuxingController.prototype.attachMediaElement = function(mediaElement) {
...
TransmuxingController.prototype._onInitChunkArrival = function (data, byteStart) {
    ...
    if (byteStart > 0 ) {
        ...
        consumed = this._demuxer.parseChunks(data, byteStart);
    } else if ((probeData = CMSDemuxer.probe(data)).match || true) {
        ...
        consumed = this._demuxer.parseChunks(data, byteStart);
    } else if ((probeData = CMSIMGDemuxer.probe(data)).match) {
        this._demuxer = new CMSIMGDemuxer(probeData, this._config);

        if (!this._remuxer) {
            this._remuxer = new IMGRemuxer(this._config);
        }
        ...
        if ( this._mediaElement ) {
            this._remuxer.attachMediaElement(this._mediaElement);
        }
        if ( this._imgElement ) {
            this._remuxer.attachMediaElement(this._imgElement);
        }        
